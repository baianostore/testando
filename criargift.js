    ...
    run: async(client, message, args) => {
      ...
      const row = new Discord.MessageActionRow()
        ...
        const embed = await message.reply({ embeds: [new Discord.MessageEmbed()
          .setTitle(`${config.get(`title`)} | Criação de Gift`)
          .addField(`<:caixa:1093295940951093259> | Estoque:`, `Nenhum...`)
          .addField(`<a:apresente_gift:1104851040119042118> | Código:`, `${gerado}`)
        ...
        const interação = embed.createMessageComponentCollector({ componentType: "BUTTON", });
          interação.on("collect", async (interaction) => {
           ...
           if (interaction.customId === "concluir") {
             ...
             message.channel.send(`<a:carregando:1069285457218773092> | Envie os novos estoques no chat!`).then(msg => {
             ...
             collector.on("collect", message => {
               ...
               for (let i = 0; i != estoque.length; i++) {
                 ...
                 if(i + 1 === estoque.length) {
                   ...
                   msg.edit(`<a:Sim:1069123911867961344> | Gift \`${gerado}\`\ Criado com sucesso!`)
                   const embednew = new Discord.MessageEmbed()
                     .setTitle(`${config.get(`title`)} | Criação de Gift Card`)
                     .addField(`<:caixa:1093295940951093259> | Estoque:`, `${texto}`)
                     .addField(`<a:apresente_gift:1104851040119042118> | Código:`, `${gerado}`)
           ...
           if (interaction.customId === "cancelar") {
             embed.delete()
             message.channel.send(`<a:No:1069123947897036880> | Cancelado...`)
           }
         })
       }
     };